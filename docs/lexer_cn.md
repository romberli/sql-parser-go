...
# demo
词法分析器可通过Flex等工具来实现, Flex是生成程序的程序, 只要定义好正则表达式, Flex可自动生成对应的词法分析器. NFA到DFA的转换是这类工具的核心功能, 不过从业界来看, 大多数语言的编译器或解释器选择了纯手工的方式来打造词法分析器, 以更多地进行针对性的优化来提高性能. 不同的编译器或解释器选择了不同的有限自动机来实现词法分析, 本demo根据上述原理分别实现了基于NFA与DFA的词法解析器.
